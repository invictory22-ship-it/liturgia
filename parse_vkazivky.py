...
# на 14.06 → зачала з'їжджали в notes). Прибираємо ЦІЛИМИ рядками ДО склейки.
COLON = re.compile(r'^(Православна Церква України|Богослужбові вказівки \d{4}|'
                   r'СІЧЕНЬ|ЛЮТИЙ|БЕРЕЗЕНЬ|КВІТЕНЬ|ТРАВЕНЬ|ЧЕРВЕНЬ|ЛИПЕНЬ|СЕРПЕНЬ|'
                   r'ВЕРЕСЕНЬ|ЖОВТЕНЬ|ЛИСТОПАД|ГРУДЕНЬ|\d{1,3})\s*$')

def strip_colontitul(seg):
    return '\n'.join(ln for ln in seg.split('\n') if not COLON.match(ln.strip()))

# Інлайн-залишки колонтитула (де pdftotext зліпив підвал з текстом в один рядок).
# Ці фрази в тексті вказівок трапляються ЛИШЕ як колонтитул, тож прибрати безпечно.
